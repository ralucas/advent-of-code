...
func (d *Day) PrepareData(filepath string) {
	...
	for _, s := range data {
		...
		pA := arrayutil.MapToInt(strings.Split(points[0], ","))
		pB := arrayutil.MapToInt(strings.Split(points[1], ","))
		ptA := NewPoint(pA[0], pA[1])
		ptB := NewPoint(pB[0], pB[1])
		lb := NewLineBuilder(ptA, ptB)
		d.lineBuilders = append(d.lineBuilders, lb)

		if x := mathutil.Max(pA[0], pB[0]); x > d.maxX {
			d.maxX = x
		}

		if y := mathutil.Max(pA[1], pB[1]); y > d.maxY {
			d.maxY = y
		}
	}
