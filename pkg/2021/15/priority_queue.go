...
func NewPriorityQueue(k PriorityQueueKind) *PriorityQueue {
...
func (pq *PriorityQueue) Heap() []string {
	sHeap := make([]string, 0)

	for _, val := range pq.heap {
		s := fmt.Sprintf("%d|%d=%d", val.node.Row(), val.node.Col(), val.weight)
...
func (pq *PriorityQueue) Pop() (*Node, int) {
	if len(pq.heap) == 0 {
		return nil, 0
	}
	...
	return val.node, val.weight
}

func (pq *PriorityQueue) Insert(node *Node, weight int) {
	v := value{node, weight}
